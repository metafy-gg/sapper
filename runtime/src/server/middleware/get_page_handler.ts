 ...
 */
function resolveOption(inputs: Array<any>, context?: object) {
...
export function get_page_handler(
  ...
  const template = dev ? () => read_template(src_dir) : ((str) => () => str)(read_template(build_dir));
        ...
        preloaded: `[${preloaded
          .map((data) =>
            try_serialize(data, (err) => {
              console.error(
                `Failed to serialize preloaded data to transmit to the client at the /${segments.join('/')} route: ${
                  err.message
                }`
              );
              console.warn(
